...
import { Swords, ArrowRight, Clock, Flame, CheckCircle2, XCircle, Volume2, VolumeX, Home, Loader2 } from "lucide-react";
...
function DuelGame({ questions, duelId }: { questions: Question[]; duelId: string }) {
    const { state, start, answer, next } = useQuiz(questions);
    ...
    // Save duel score when finished
    useEffect(() => {
        if (state.status !== "finished" || !user || savedRef.current) return;
        savedRef.current = true;
        const supabase = createClient();
        // Check if user is challenger or opponent
        supabase.from("duels").select("*").eq("id", duelId).single().then(async ({ data: duel }) => {
            if (!duel) return;
            if (duel.challenger_id === user.id) {
                await supabase.from("duels").update({ challenger_score: state.score, status: duel.opponent_score !== null ? "completed" : "active" }).eq("id", duelId);
            } else {
                await supabase.from("duels").update({ opponent_id: user.id, opponent_score: state.score, status: duel.challenger_score !== null ? "completed" : "active" }).eq("id", duelId);
            }
        });
    }, [state.status, user, state.score, duelId]);
        ...
        return (
            <div className="container mx-auto px-4 py-16 text-center max-w-lg">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    ...
                    <h1 className="font-heading text-3xl font-bold mb-3">Duel ⚔️</h1>
                    <p className="text-muted-foreground mb-2">{questions.length} questions</p>
                    ...
                    <Button size="xl" onClick={start} className="gap-2">
                        Commencer le duel <ArrowRight className="h-5 w-5" />
                    </Button>
                </motion.div>
            </div>
        );
        ...
        return (
            <div className="container mx-auto px-4 py-16 text-center max-w-lg">
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    ...
                    <h2 className="font-heading text-3xl font-bold mb-2">Duel terminé !</h2>
                    <div className="glass-card rounded-2xl p-6 mb-6">
                        ...
                        <p className="text-sm mt-2 text-muted-foreground">Ton score a été enregistré.</p>
                    </div>
                    ...
                    <div className="flex gap-3 justify-center">
                        <Link href="/duel/create"><Button variant="outline" className="gap-2"><Swords className="h-4 w-4" /> Nouveau duel</Button></Link>
                        <Link href="/"><Button className="gap-2"><Home className="h-4 w-4" /> Accueil</Button></Link>
                    </div>
                </motion.div>
            </div>
        );
...
export default function DuelPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            if (!duel) { setError("Duel introuvable"); setLoading(false); return; }

            // Fetch questions by IDs
            const { data: qs } = await supabase.from("questions").select("*").in("id", duel.question_ids);
            if (qs) {
    ...
    if (error || questions.length === 0) {
        return (
            <div className="container mx-auto px-4 py-16 text-center">
                <p className="text-muted-foreground">{error || "Aucune question trouvée pour ce duel."}</p>
                <Link href="/duel/create"><Button variant="outline" className="mt-4">Créer un duel</Button></Link>
            </div>
        );
    }

    return <DuelGame questions={questions} duelId={id} />;
}
